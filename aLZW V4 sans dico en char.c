...
void compress(FILE *input_file,FILE *output_file) {
    ...
    char str[12];
    char chn[24],chn_sauv[24],last_valid[100];
    int i;
    int Trouve;
    int last_code = 0;
    char dictionary[DICTIONARY_SIZE][12];
   ...
   // Initialise le dictionnaire avec les codes ascii
    ...
    while ( ! feof(input_file) && next_code < DICTIONARY_SIZE) { //while not end of file do
        input_symbol = getc(input_file);
        printf("input_symbol %d\n",input_symbol);
        if(input_symbol == 255) break; //doutes
        //Conversion en chaines de caractères
        str[0] = input_symbol;
        str[1] = '\0';
        strcpy(chn, last_valid);
        strcat(chn, str);
        //if (next_code<270) fprintf(stderr,"get chn(%s) str(%s) last_valid(%s)\n",chn,str,last_valid);
