...
void compress(FILE *input_file,FILE *output_file) {
    ...
    memset(chn_dico,0,sizeof(uint32_t));
    ...
    bit_put(bf, 256, current_code_size);
    ...
    while ( ! feof(input_file)) { //tant que la fin du fichier n'est pas atteinte
        ...
        else 
        {
            ...
            next_code++;
            /*if (next_code == 512) printf("Depassement de 9 bits\n");
            if (next_code == 1024) printf("Depassement de 10 bits\n");
            if (next_code == 2048) printf("Depassement de 11 bits\n");
        ...
        if (next_code == 4096) {
            printf("Réinit du dico en compression\n");
            Liberation(racine);
            racine = CreationFeuille(-1);
            for (i = 0; i < 256; i++) {
                chn_dico[0] = i;
                insert(racine, chn_dico ,i, 1);
            }
...
void decompress(FILE *input_file,FILE *output_file) {
   ...
   //une boucle qui prend chaque ligne les unes apres les autres jusqu'à la fin du fichier
    while (bit_get(bf, &code, current_code_size) != EOF) {
        ...
        if (next_code == 4096) {
            printf("Réinit du dico en décompression\n");
            Liberation(racine);
            racine = CreationFeuille(-1);
            for (i = 0; i < 256; i++) {
                chn_dico[0] = i;
